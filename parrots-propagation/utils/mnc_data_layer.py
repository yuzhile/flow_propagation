...
class MNCDataLayer(caffe.Layer):
    ...
    def _get_next_minibatch(self):
        ...
        thread_id = caffe.getThreadId()
        data_step_size = thread_num
        if self._cur + data_step_size >= len(self._roidb):
            self._shuffle_roidb_inds()
        db_inds = self._perm[self._cur]
        #print('mnc_data_layer:139,thread_id:{},thread_num:{},self._cur:{}'.format(thread_id,thread_num,self._cur))
        self._cur += data_step_size
