...
class CityscapesSequenceLmdbReader:
    '''
    ...
    def config(self,cfg):
        ...
        self.image_env = lmdb.open('{}/trainval_lmdb'.format(self.data_dir),readonly=True)
        ...
        self.error_list = open('{}/error_list.txt'.format(self.data_dir),'r').read().splitlines()
...
#                print 'handling {}:{}'.format(i,idxs)
                ...
                current_image, key_image ,label = self._lmdb_load_image_label(idxs[0],idxs[1],idxs[2])
                #apply mirror transformer
                if self.mirror and np.random.randint(2):
...
#                print current_crop_img.shape,key_crop_img.shape,label_shrink.shape,label_mask.shape
    ...
    def _lmdb_load_image_label(self,idx,split,position):
        '''
        Load input image and preprocess for parrots
        - cast to float
        '''
        ...
        raw_data = np.fromstring(self.image_txn.get(idx),dtype='uint8')
        ndata = raw_data.reshape(self.h,self.w,self.c_image)
        image = ndata[:,:,:3]
        ...
        for i in range(10):
            ...
            if current_name in self.error_list:
                continue
            
            current_raw_im = np.fromstring(self.sequence_txn.get(current_name),dtype='uint8')
            current_im = current_raw_im.reshape(self.h,self.w,self.c_sequence)
            current_im = self.half_crop_image(current_im,position,label=False)
            current_in_ = np.array(current_im,dtype=np.float32)
            break
...
if 'TEST_CITYSCAPES' in globals():
    ...
    reader.register_pyreader(CityscapesSequenceLmdbReader,'CityscapesSequenceLmdb')
